...
### 5. [Undocked 구성 요소](#5)
### 6. [게임 시스템 디자인](#6)
#### &emsp;&emsp; 6-1. [게임 오브젝트 분해](#6-1)
#### &emsp;&emsp; 6-2. [오브젝트 속성(파라미터)](#6-2)
#### &emsp;&emsp; 6-3. [오브젝트 행동](#6-3)
#### &emsp;&emsp; 6-4. [오브젝트 상태](#6-4)
#### &emsp;&emsp; 6-5. [게임의 규칙](#6-5)
#### &emsp;&emsp; 6-6. [게임에서 사용될 공식](#6-6)
### 7. [개발 요구 사항 흐름도](#7)
#### &emsp;&emsp; 7-1. [요구사항](#7-1)
#### &emsp;&emsp; 7-2. [시간별 흐름도 Flowchart](#7-2)
#### &emsp;&emsp; 7-3. [키보드 이벤트에 대한 흐름도](#7-3)
#### &emsp;&emsp; 7-4. [용어정리](#7-4)
### 8. [스토리보드](#8)
...
## 2. 플레이어 오브젝트 속성(파라미터)<a name=6-2></a>
...
## 1. 요구사항 <a name=7-1></a>
### 씬)
- 타이틀 화면, 게임 화면, 게임 오버 화면, 스테이지 클리어 화면, 게임 클리어 화면으로 구성되었다.
- 타이틀 화면의 배경은 게임 오브젝트를 사용해 혼란스러운 실내 공간을 연출하고 화면 오른쪽에 게임 제목이 적힌 텍스트를 배치한다.   
- 타이틀 화면 제목 텍스트 하단에 게임 시작, 환경 설정, 나가기 버튼을 배치한다.
- 게임 시작 버튼 클릭 시 게임 화면으로 전환한다.
- 환경 설정 버튼 클릭 시 환경 설정 UI가 생성된다.
- 타이틀 화면에서 나가기 버튼을 클릭 시 게임이 종료되도록 한다.
- 게임 오버 화면에 다시 시작 버튼, 타이틀 버튼을 배치한다.
- 다시 시작 버튼을 누르면 게임을 처음부터 다시 시작하고 타이틀 버튼을 누르면 타이틀 화면으로 전환한다.
- 스테이지 클리어 화면은 다음 스테이지 버튼과 타이틀 버튼을 배치한다.
- 게임 클리어 화면은 타이틀 버튼을 배치한다. 

### 플레이)
- 플레이어는 1인칭 시점으로 하고 화면 오른쪽에 플레이어 무기와 손이 보이도록 한다.
- 플레이어는 W, A, S, D 키를 입력하여 플레이어 오브젝트를 이동한다.
- 플레이어가 이동 중일 때 Left Shift 키를 입력해 달릴 수 있다.
- 플레이어는 스페이스 바를 입력해 점프할 수 있다.
- 플레이어는 마우스 드래그를 하여 시야를 회전한다.
- 플레이어는 달리지 않는 상태에서 마우스 왼쪽 버튼을 클릭해 총을 발사할 수 있고, 
  R키를 입력해 총알을 재장전할 수 있다.
- 플레이어는 E 키를 입력해 소형 폭탄 아이템을 포물선으로 던질 수 있다.
- 플레이어는 Q 키를 입력해 손전등의 빛을 끄고 킬 수 있다.
- 플레이어는 상호작용 가능한 거리에서 오브젝트에 크로스헤어를 조준하여 F 키를 입력하면
  각 오브젝트에 맞는 상호작용을 할 수 있다.
- 적 오브젝트들에 웨이포인트를 설정해 랜덤으로 지정하여 패트롤하도록 한다. 
- 적 오브젝트들의 감지 범위 내에 플레이어가 있다면 플레이어를 향해 돌진한다.
- 적 오브젝트들의 공격 사거리 내에 플레이어가 있다면 공격 모션을 재생한다.
- 적 오브젝트들은 스테이지가 올라갈수록 높은 체력과 공격력을 가진다.
- 플레이어가 적 공격에 맞는다면 각 적 오브젝트들의 공격력만큼 체력이 감소한다.
- 플레이어가 적 공격에 맞으면 약 0.5초 화면이 붉게 변하는 피격효과를 생성한다.
- 플레이어의 체력이 0이하라면 게임 오버 씬으로 전환된다. 
- 플레이어 총에 명중한 적은 체력이 1감소한다.
- (1,2 스테이지)잠겨있는 문 오브젝트를 열기 위해 숫자패드 오브젝트를 배치해 비밀번호를 입력하도록 한다.
- 비밀번호는 게임이 실행될 때마다 랜덤으로 4자리수를 설정한다.
- 맵에 존재하는 랩탑 오브젝트를 상호작용하면 비밀번호 UI를 활성화한다.
- 잠겨있는 문을 열고 들어가면 스테이지 클리어 화면으로 전환한다.
- 부서지는 벽을 향해 소형 폭탄을 던져 맞추면 벽이 부서지도록 한다(애니메이션 재생).
- (3스테이지) 우주선의 배터리를 모두 찾아 충전을 완료하는 것을 최종 탈출 조건으로 한다.
- 배터리 오브젝트를 상호작용하여 획득하면 해당 오브젝트를 삭제하고 1개를 획득한다.
- 배터리 디스플레이 오브젝트를 상호작용하여 현재 배터리 충전량 UI를 생성한다.
- 우주선 오브젝트를 상호작용하면 배터리 개수가 1개씩 감소하고 배터리 충전량이 20%씩 증가한다. 
- 충전을 완료하면 게임 클리어 화면으로 전환된다.

### 맵)
- 행성 탐사 기지로 배경으로 한다.
- 사고를 당해 혼란스럽게 어질러진 실내 분위기를 연출한다.
- 3개의 스테이지를 제작하고 각 스테이지에 다른 적 오브젝트들을 생성하도록 한다.
- 각 스테이지에는 막다른 길이 존재하며 부서지는 벽 오브젝트를 배치한다.

### UI)
- 게임 화면 왼쪽 하단에 플레이어의 체력 UI를 “HP 플레이어 체력” 형태로 배치한다.
- 게임 화면 오른쪽 하단에 총알 개수 UI를 “현재 탄창 총알 개수 / 전체 탄창 개수” 형태로 배치한다. 
- 화면 중앙에 점 모양 크로스헤어를 배치한다.
- 오브젝트를 상호작용할 때 크로스헤어 위에 텍스트 UI를 생성한다.
	  ex) 획득하기[F], 소형 폭탄 1개 획득, 
- 비밀번호 UI는 검은 배경으로 하고 중앙에 초록색 비밀번호 텍스트를 출력한다.
	  ex) “password:１２３４” 
- 배터리 충전량 UI는 노란색 배경으로 하고 중앙에 충전량을 텍스트로 출력하고 텍스트 하단에 
  충전 게이지 바를 배치하여 충전 시 충전량과 게이지가 증가하도록 한다. 
- 배터리 개수 UI는 획득한 배터리 오브젝트의 개수가 1개 이상일 때 화면 왼쪽 상단에 생성하며
  배터리 이미지 X 개수 형태로 배치한다.
- 게임 화면에서 Esc 키를 입력하면 메뉴 UI를 생성한다.
- 메뉴 UI 왼쪽에 Return, Setting, Exit 버튼을 배치한다.
- 메뉴 UI 오른쪽에 조작법 이미지를 배치한다. 

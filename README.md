...
22. 플레이어는 상호작용 가능한 거리에서 오브젝트에 크로스헤어를 조준하여 F 키를 입력하면 각 오브젝트에 맞는 상호작용을 할 수 있다.
23. 1스테이지 적 오브젝트 2개를 제작한다. (집게 외계 종족 일반, 외계 돌연변이)
24. 2스테이지 적 오브젝트 2개를 제작한다. (집게 외계 종족 강화, 인간 숙주 돌연변이)
25. 3 스테이지 적 오브젝트 3개를 제작한다. (렙틸리언, 타이탄 알파)
26. 적 오브젝트들에 웨이포인트를 설정해 랜덤으로 지정하여 패트롤하도록 한다.
27. 적 오브젝트들의 감지 범위 내에 플레이어가 있다면 플레이어를 향해 돌진한다.
28. 적 오브젝트들의 공격 사거리 내에 플레이어가 있다면 공격 모션을 재생한다.
29. 적 오브젝트들은 스테이지가 올라갈수록 높은 체력과 공격력을 가진다.
30. 적 오브젝트의 애니메이션은 기본, 걷기, 돌진, 피격, 공격, 사망 모션을 사용한다.
31. 플레이어가 적 공격에 맞는다면 각 적 오브젝트들의 공격력만큼 체력이 감소한다.
32. 플레이어가 적 공격에 맞으면 약 0.5초 화면이 붉게 변하는 피격효과를 생성하고 카메라 흔들림 효과를 생성한다.
33. 체력이 감소된 상태에서 힐팩 오브젝트를 상호작용하면 체력을 회복한다.
34. 플레이어의 체력이 0이하라면 게임 오버 씬으로 전환된다.
35. 플레이어 총에 명중한 적은 체력이 1감소한다.
36. (1,2 스테이지)잠겨있는 문 오브젝트를 열기 위해 숫자패드 오브젝트를 배치해 비밀번호를 입력하도록 한다.
37. 비밀번호는 게임이 실행될 때마다 랜덤으로 4자리수를 설정한다.
38. 맵에 존재하는 랩탑 오브젝트를 상호작용하면 비밀번호 UI를 활성화한다.
39. 잠겨있는 문을 열고 들어가면 스테이지 클리어 화면으로 전환한다.
40. 부서지는 벽을 향해 소형 폭탄을 던져 맞추면 벽이 부서지도록 한다(애니메이션 재생).
41. (3스테이지) 우주선의 배터리를 모두 찾아 충전을 완료하는 것을 최종 탈출 조건으로 한다.
42. 총알 오브젝트를 상호작용하면 총알 10개를 획득한다.
43. 배터리 오브젝트를 상호작용하여 획득하면 해당 오브젝트를 삭제하고 1개를 획득한다.
44. 배터리 디스플레이 오브젝트를 상호작용하여 현재 배터리 충전량 UI를 생성한다.
45. 우주선 오브젝트를 상호작용하면 배터리 개수가 1개씩 감소하고 배터리 충전량이 20%씩 증가한다.
46. 충전을 완료하면 게임 클리어 화면으로 전환된다.

### 맵)
47. 행성 탐사 기지로 배경으로 한다.
48. 사고를 당해 혼란스럽게 어질러진 실내 분위기를 연출한다.
49. 1 스테이지를 제작한다. (Section 1)
50. 2 스테이지를 제작한다. (Section 2)
51. 3 스테이지를 제작한다. (Section 3)
52. 각 스테이지에 다른 적 오브젝트들을 생성하도록 한다.
53. 각 스테이지에는 막다른 길이 존재하며 부서지는 벽 오브젝트를 배치한다.

### UI)
54. 게임 화면 왼쪽 하단에 플레이어의 체력 UI를 “HP 플레이어 체력” 형태로 배치한다.
55. 게임 화면 오른쪽 하단에 총알 개수 UI를 “현재 탄창 총알 개수 / 전체 탄창 개수” 형태로 배치한다.
56. 화면 중앙에 점 모양 크로스헤어를 배치한다.
57. 오브젝트를 상호작용할 때 크로스헤어 위에 텍스트 UI를 생성한다. ex) 획득하기[F], 소형 폭탄 1개 획득,
58. 비밀번호 UI는 검은 배경으로 하고 중앙에 초록색 비밀번호 텍스트를 출력한다. ex) “password:１２３４”
59. 배터리 충전량 UI는 노란색 배경으로 하고 중앙에 충전량을 텍스트로 출력하고 텍스트 하단에  충전 게이지 바를 배치하여 충전 시 충전량과 게이지가 증가하도록 한다.
60. 배터리 개수 UI는 획득한 배터리 오브젝트의 개수가 1개 이상일 때 화면 왼쪽 상단에 생성하며 배터리 이미지 X 개수 형태로 배치한다.
61. 게임 화면에서 Esc 키를 입력하면 메뉴 UI를 생성한다.
62. 메뉴 UI 왼쪽에 Return, Setting, Exit 버튼을 배치한다.
63. 메뉴 UI 오른쪽에 조작법 이미지를 배치한다. 
